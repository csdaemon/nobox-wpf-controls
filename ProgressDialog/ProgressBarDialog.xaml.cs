...
namespace Nobox.WPF.Controls.ProgressDialog
{
	public partial class ProgressBarDialog : Window
	{
		...
		public ProgressBarDialog(ProgressDialogSettings settings)
		{
			InitializeComponent();
		...
		public ProgressDialogResult Execute(Action operation)
        {
			return Execute((object)operation);
        }
		...
		public ProgressDialogResult Execute(object operation)
		{
			...
			_worker.RunWorkerCompleted +=
				(s, e) => {
					...
					this.Dispatcher.BeginInvoke(DispatcherPriority.Send, (SendOrPostCallback)delegate {
						_isBusy = false;
						Close();
					}, null);
				};
		...
		{
			ProgressBarDialog dialog = new ProgressBarDialog(settings);
			dialog.Owner = owner;
